...
/*FIXME remove if not needed.                        if (bError.code != BError.Code.NO_LOGIN_INFO)
    ...
    public void passwordLogin() {
        ...
        BNetworkManager.sharedManager().getNetworkAdapter()
            ...
            @Override
            public void onFail(BError bError) {
                if (bError.code == BError.Code.NO_VERIFY_EMAIL)
    ...
    public void register(){
        ...
        BNetworkManager.sharedManager().getNetworkAdapter()
            ...
            public void onDone(Object o) {
                //afterLogin();
                Toast.makeText(that, "Verification email sent", Toast.LENGTH_LONG).show();
                dismissProgDialog();
            }
        }).fail(new FailCallback<BError>() {
            @Override
            public void onFail(BError bError) {
                Toast.makeText(that, "Failed to send verification email", Toast.LENGTH_LONG).show();
                dismissProgDialog();
            }
        });
    }
/*
    ...
    protected boolean checkFields(){
        if (etEmail.getText().toString().isEmpty())
        {
            ...
            String[] split = etEmail.getText().toString().split("@");
            if (split.length != 2 || split[0].isEmpty() || !split[1].equalsIgnoreCase("post.bgu.ac.il")) {
