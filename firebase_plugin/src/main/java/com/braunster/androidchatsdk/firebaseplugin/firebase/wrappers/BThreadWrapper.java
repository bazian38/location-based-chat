...
//            earliestMessage = messages.get(0);
    ...
    Map<String, Object> serialize(){
        ...
        nestedMap.put(BDefines.Keys.BDescription, this.model.getDescription());
    ...
    public static BThreadWrapper initWithEntityId(String key) {
        BThread model = DaoCore.fetchOrCreateEntityWithEntityID(BThread.class, key);
        return initWithModel(model);
    }

    public static BThreadWrapper initWithModel(BThread model) {
        return new BThreadWrapper(model);
    }

    public Promise<BThread, BError, Void> once(){

        final Deferred<DataSnapshot, BError, Void> deferred = new DeferredObject<>();

        final Deferred<BThread, BError, Void> promise = new DeferredObject<>();
    ...
    private DatabaseReference ref() {
        return FirebasePaths.threadRef(entityId);
    }
